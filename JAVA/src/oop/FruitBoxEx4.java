...
class AppleComp implements Comparator<Apple4> { // <? super Apple4>
	public int compare(Apple4 t1, Apple4 t2) {  
		return t2.weight - t1.weight;   // 내림차순
	}
}

class GrapeComp implements Comparator<Grape4> {  // <? supper Grape4>
...
class FruitBoxEx4 {
	public static void main(String[] args) {
		...
		Collections.sort(grapeBox.getList(), new GrapeComp());  //new FruitComp()으로 바꿔도 차순만 바뀔 뿐 실행 잘 된다.
...
//  매개변수 타입이 Comparator<? super Apple>이라는 의미는 Comparator의 타입 매개변수로
// Apple 과 그 조상이 가능하다 는 뜻
// Comparator<Apple>,Comparator<Fruit>,Comparator<Object> 가능
// 와일드 카드가 어려우면 무시하고 Comparator<T>라고 생각하기
