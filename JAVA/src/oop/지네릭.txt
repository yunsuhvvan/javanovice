지네릭스란
 ...
 1. 지네릭 타입이 다른 것만으로는 오버로딩이 성립하지 않는다.
  ...
  지네릭 메서드( 지네릭 타입 선언 위치: 반환타입 바로 앞)
  
  1.지네릭 클래스에 정의 된 타입 != 지네릭 메서드에 정의된 타입 (같은 문자를 사용해도 같은것이 아니다)
  2.static 멤버에는 타입 매개변수를 사용할 수 없지만, 메서드에 지네릭 타입을 선언하면 사용가능 (lv 라고 생각하자) -> 메서드 내에만 지역적으로 사용되므로 메서드가 static이건 아니건 상관 X
  3. 대부분의 경우 컴파일러가 타입을 추정할 수 있기때문에 생략해도 된다.
    static -> 클래스레벨 X
    static -> 메서드레벨 O


지네릭 타입의 형변환
  1. 지네릭 타입과 넌지네릭 타입간의 형변환은 항상 가능 But 경고
  2. 와일드카드를 쓰면 매개변수의 다형성으로 형변환 가능.
   
 
 지네릭 타입의 제거
 1. 컴파일러는 지네릭 타입을 이용해서 소스파일 체크 --> 필요한 곳에 형변환을 넣어준다 --> 지네릭 타입 제거
 2. 컴파일러 후에 지네릭을 제거하기때문에 제거하고나서  타입이 일치하지 않으면 형변환을 해줘야한다.
